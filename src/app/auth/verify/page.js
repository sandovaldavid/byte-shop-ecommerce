...
function VerifyPageContent() {
	...
	const statusConfig = {
		...
		success: {
			icon: (
				<AiOutlineCheckCircle className='w-20 h-20 text-success animate-scale-in' />
			),
		...
		error: {
			icon: (
				<AiOutlineCloseCircle className='w-20 h-20 text-error animate-shake' />
			),
	...
	return (
		<div className='min-h-screen bg-dark flex items-center justify-center relative overflow-hidden py-16'>
			...
			<div
				className={`absolute top-0 right-0 w-96 h-96 rounded-full filter blur-3xl transition-colors duration-500
                ${
					status === 'loading'
						? 'bg-secondary/10'
						: status === 'success'
			...
			<div
				className={`absolute bottom-0 left-0 w-96 h-96 rounded-full filter blur-3xl transition-colors duration-500
                ${
					status === 'loading'
						? 'bg-accent1/10'
						: status === 'success'
			...
			<div className='relative w-full max-w-md p-8 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10'>
				<div className='flex flex-col items-center justify-center space-y-6 text-center'>
					...
					<p className='text-light/70'>{current.message}</p>
